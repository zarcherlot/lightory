...
架构工作台负责功能模块、成果、子系统、交付连接、Agent 分配和草案验收。交付连接同时提供数据流和依赖依据；系统自动显示先后关系和可并行批次，不建设第二张计划 DAG。
...
基础模拟只消费已验收蓝图编译得到的 `RobotPlan`，确定性执行前进、转向、停止和语音；显示单步事件、运动轨迹、语音、目标到达及边界/障碍碰撞证据。孩子决定何时准备、单步或运行到底；蓝图或场景改变后必须重新准备。模拟期不调用 Agent 或真实 Robot API。
...
| 带箭头的连线 | 模块间的成果或完成信号交付 |

连线建立后，孩子回答“上一个模块完成后，要把什么交给下一个？”系统将其绑定为成果或完成信号。执行依赖由交付连接推导：依赖均满足且彼此没有依赖的模块可以并行。
...
type BlueprintNodeKind = 'start' | 'function' | 'artifact' | 'container';
type BlueprintRelation = 'handoff';
type HandoffKind = 'artifact' | 'completion';
...
旧 `data` 边迁移为成果交付，旧 `trigger` 边迁移为完成信号。旧存档中的 `planSteps` 仅为兼容读取，不再写入；旧任务的 `plan` 阶段门槛迁移为 `build`。迁移不得丢失原始笔迹、Agent 交付或评审记录。
