...
4. 确认 Agent 对任务的理解。
5. 验收或退回 Agent 的方案。
6. 定义模块交付，查看系统推导的先后与并行关系，并主动发起构建。
7. 自由布置试验场，运行模拟并观察架构如何影响小车行为。
...
- **平板 App 是工程工作台**：孩子在这里定义目标、画架构、定义交付、组织 Agent、发起构建、验收和调试。产品的主要学习价值发生在 App 中。
...
纵向原型先使用与真实 Tool 语义一致的模拟器；真实小车接入延后，但领域模型和可执行蓝图不得为模拟器特化。

### 2.2 一个通用工作台，而非一个任务一个前端包

- “家庭寻宝”只是第一条验收任务，不是页面结构或代码分支。
- 后续“博物馆导览”“校园任务”等必须复用同一个工作台、工程对象、Agent 协作和调试流程。
- 新任务以可独立加载的 JSON 数据定义目标背景、可用 Tool、Agent、阶段门槛、预设故障和成功条件。
- 任务定义不得包含 React 组件名、屏幕坐标、CSS 或页面跳转逻辑。
- 前端只渲染通用对象：目标、节点、交付连接、Agent、构建批次、场景图元、测试结果和调试记录。
...
2. **家庭寻宝毕业任务**：孩子综合使用目标、架构、分工、交付、构建、测试和调试。
...
- 定义模块之间交付什么；查看系统由连接推导出的先后和并行关系，决定何时开始或重新构建。
- 验收或退回交付，预测运行结果并发起实验。
- 对照预期与实际定位问题，选择修复方案并重新验证。

界面必须留下孩子的工程足迹：自己提出的目标、做出的决定、退回的交付、发现的错误和完成的修复。完成度不得只按最终结果评价。

孩子的原始表达必须保留为工程证据，包括文字、未来的语音转写和画布元素引用。AI 可以把表达整理成“目标、输入、期望结果、安全约束和可观察验收标准”的候选合同，但必须展示“你说的、工程师理解的、仍不清楚的”，由孩子确认或修改。AI 不得静默创建模块、连接架构、补写成功标准或把默认值冒充孩子的决定。
...
- 介绍自己的 Tool、输入、输出和限制。
- 询问缺失信息并复述对任务的理解。
- 提交局部设计、受约束的结构化参数或测试结果。
- 解释失败原因，按孩子的修改意见重新交付。
- 明确表示不确定、能力不足或依赖缺失。

### 3.3 Agent 工程师不能做什么

- 不得替孩子定义总目标、总体架构或完整可执行蓝图。
- 不得自行聘用其他 Agent、改变分工或扩大任务范围。
- 不得未经确认写入正式蓝图、接受自己的交付或启动小车。
- 不得绕过 Tool 权限、安全限制、构建门槛或紧急停止。
...
等待分配 -> 复述任务 -> 等待孩子确认 -> 工作中
        -> 批准任务并派工 -> 提交方案 -> 等待验收 -> 通过或退回
```

- Agent 的交付先以半透明草案显示，不直接写入正式蓝图。
- 被退回时保留上一版及孩子的修改意见。
- 自动测试和教学脚本使用确定性 Agent Provider；生产 Provider 只能在工程构建期生成符合 Schema 的局部交付。
- 孩子批准复述时同时派工；依赖未满足的任务自动排队，满足后自动开始。孩子修改已验收架构、合同或上游交付后，系统先展示受影响节点及其下游，再由孩子确认重新派工。
...
1. 孩子绘制大框并命名为“寻宝系统”，在其中建立“移动”和“语音”模块。
2. 将“路线工程师”和“语音工程师”分配到相应模块。
3. 孩子用自然语言、文字字段或画布引用定义合同；Agent 分别复述，孩子确认后才开始工作。
4. 孩子接受局部草案，并用交付连接说明“移动完成后把到达结果交给语音”。
5. 系统显示由连接推导出的依赖和可并行批次，不要求孩子再画一张计划图。
6. 孩子进入独立试验场，用通用图元自由布置起点、目标和障碍，并自行命名为“小车”“宝箱”“椅子”等。
7. 孩子写下预期并点击“开始构建”；系统只运行待构建 Agent，显示儿童摘要和可展开的技术制品。
8. 编译器将已验收制品确定性组装成 `RobotPlan`，通过安全校验后展示儿童可读的动作预览。
9. 启动模拟，节点依次亮起，交付沿连线流动，小车在试验场中执行移动和语音。
10. P4 中“移动”模块因预设的错误角度在首个异常点暂停，只显示预期、实际和证据。
11. 孩子补充合同或带证据退回 Agent，确认复述后只重新构建受影响节点及其下游。
12. 重新模拟成功，界面显示本次由孩子完成的决定、诊断与修正。
...
- Agent、导师、构建详情和调试工具以浮动坞或上下文抽屉出现。
- Agent 卡片同时展示擅长能力、已知短板和“需要总工程师复核”状态；不显示本次错误答案。
- 视觉采用深色工程蓝图、青色光效和 2.5D 层次；首版不实现三维编辑器。

### 5.2 架构工作台与独立试验场

架构工作台负责流程控制、功能模块、子系统、模块连线、Agent 分配和草案验收。连线统一为“触发信号”或“传递消息”：两者都会让下游模块自动等待并开始，传递消息还会把孩子填写的消息内容作为下游 Agent 的显式输入。系统自动显示先后关系和可并行批次，不建设第二张计划 DAG。

独立试验场负责环境建模和运行观察。它复用绘制、选择、缩放和属性编辑语言，但与架构图分开保存、分开展示，避免平板画面拥挤。P3 提供起点、目标/地标、障碍、区域和普通对象五类通用图元；孩子自由摆放、命名并填写含义，合同可以点选引用。任务不得携带专属页面或道具组件。

基础模拟只消费已验收蓝图编译得到的 `RobotPlan`，P3 首个试验场能力是平面移动模拟：确定性执行前进、转向、停止和语音；显示单步事件、运动轨迹、语音、目标到达及边界/障碍碰撞证据。孩子决定何时准备、单步或运行到底；蓝图或场景改变后必须重新准备。模拟期不调用 Agent 或真实 Robot API。后续复杂任务通过新增试验场能力插件扩展，例如视觉识别样例、抓取状态、多机器人或事件时间线，不把所有任务硬塞进平面移动模拟。

构建详情默认显示儿童可读的依赖批次、交付摘要和动作预览；技术端口、Schema、参数与原始制品按需展开。
...
| 带箭头的连线 | 模块间的触发信号或传递消息 |

连线建立后，孩子先选择“触发信号”或“传递消息”。触发信号必须填写“什么时候触发下一个模块”；传递消息必须填写“要传给下一个模块什么信息”。传递消息也会自动触发下游开始，避免孩子重复画两条线。执行依赖由连线推导：依赖均满足且彼此没有依赖的模块可以并行。
...
- 孩子点击“批准任务并派工”后才调用 Agent；不得再增加无信息、无选择的二次开工点击。修改产生的返工范围必须先显示给孩子。
- 系统只自动启动最早可执行批次；同批 Agent 可以并行提交方案，下游等待孩子接受全部必要上游方案后自动启动。
- 已验收且未变更的制品使用缓存，只重新构建待更新节点及其下游。
- Agent 只在构建期工作；模拟和真实机器人运行期只执行确定性编译、安全校验后的 `RobotPlan`。
- 运行时数据沿连线移动，当前节点发光，已完成节点保留完成态。
- 安全但不完整的架构允许模拟并显示未满足的验收项；危险动作必须在运行前拒绝。
- 孩子必须在看到结果前保存至少一项预期；首批通用标准为最终目标、语音内容、安全完成和“到达目标后再播报”。预期进入蓝图历史，AI、模拟器和比较器均无权改写。
...
任务可以改变文案、Agent、Tool、提示、成功条件和测试数据，但不能改变第 5.1—5.6 节的基础交互语法。普通任务不得提供专属道具包；新的可执行行为必须作为通用 Tool 编译器和模拟器能力注册。
...
type BlueprintNodeKind = 'start' | 'end' | 'function' | 'container';
type BlueprintRelation = 'handoff';
type HandoffKind = 'trigger' | 'message';
...
interface TaskDefinition {
  ...
  stageGates: Array<'goal' | 'architecture' | 'assignment' | 'build' | 'test'>;
...
type WorkflowNodeStatus =
  | 'waiting' | 'ready' | 'running' | 'awaiting-review'
  | 'accepted' | 'dirty' | 'blocked' | 'failed';
...
type SceneEntityKind =
  | 'robot-start' | 'target-landmark' | 'obstacle' | 'area' | 'object';
...
旧 `data` / `artifact` 边迁移为 `message`，旧 `trigger` / `completion` 边迁移为 `trigger`。旧成果节点仅作为兼容读取的中转节点保留，不再作为新建概念暴露。旧存档中的 `planSteps` 仅为兼容读取，不再写入；旧任务的 `plan` 阶段门槛迁移为 `build`。迁移不得丢失原始笔迹、Agent 交付或评审记录。

构建期 Agent 必须通过统一 Provider 返回 `AgentArtifact`。系统根据交付连接生成 `AgentWorkflow.batches`；Agent 不得返回 `RobotPlan`。编译器只读取孩子已确认的合同和已验收制品，确定性生成受限 Robot Intent/DSL，再复用现有计划构建和安全校验。首批 `movement-v1`、`speech-v1` 制品缺少明确参数时必须报错并交回孩子处理，不得由 Agent 或编译器猜测；动作预览只生成候选，不连接真实小车。

具体 Agent 框架必须位于 Lightory 编排器之下，并实现统一边界：
...
interface AgentRuntimeRequest {
  ...
  assignmentId: string;
  contract: AgentTaskContract;
  visibleArtifacts: Array<{ deliveryId: string; artifact: AgentArtifact }>;
...
`DeterministicAgentAdapter` 用于测试；未来的 Codex/OpenCode 适配器必须遵守相同接口。Lightory 只向 Agent 提供合同和交付连接允许的制品，并在框架权限之外再次校验 Tool 调用；文件、Shell、网络、子 Agent 和 Robot API 默认不可用。Agent 框架不得成为总体编排器，也不得通过看到完整蓝图来替孩子补齐缺失设计。

儿童语音需求输入只定义能力边界，P3 不绑定具体 STT/TTS 服务：

```ts
interface ChildSpeechInputAdapter {
  isAvailable(): boolean;
  transcribe(audio: Blob): Promise<{ transcript: string }>;
}
```

适配器不可用时必须明确提示并保留文字输入，不得把机器人实时音频 Tool 当作儿童语音转写接口。
...
- `WorkflowInspector`：儿童可读的依赖、并行批次、待构建范围和可展开技术详情。
- `AgentDock`：Agent 能力展示、拖放和任务复述。
- `AgentAssignmentController`：Agent 状态及交付验收。
- `AgentWorkflowRuntime`：主动构建、结构化 Provider、缓存和脏节点传播。
- `BlueprintCompiler`：将已验收结构化制品确定性组装为受安全约束的 `RobotPlan`。
- `TestFieldEditor`：通用场景图元、自由布置、命名、含义和画布引用。
- `SimulationRuntime`：基础移动与语音的确定性运行、数据流动画和故障暂停。
...
- 单元测试：几何识别、坐标变换、笔画分组、包含关系、交付兼容、拓扑批次、循环检测、脏节点传播、编译和状态机。
- 组件测试：候选确认/拒绝、Agent 拖放/验收、撤销/重做、试验场编辑、主动构建及儿童/技术详情切换。
- 可执行蓝图测试：缺少必要输入或接口不兼容时阻止构建；独立节点并行；修改一个合同只重建该节点及下游；Agent 输出不能绕过编译器进入 `RobotPlan`。
- 试验场测试：前进/转向/停止/语音确定性执行；目标到达、旋转障碍和边界碰撞可观察；并行底盘动作冲突时停止；危险参数在运行前拒绝。
- 预期对照测试：未记录预期不能开始实验；目标、语音、安全和先到达后播报逐项显示实际证据；串行反转或同批并行均不得误判为正确顺序；首个差异点暂停。
- P4 调试闭环测试：修复后必须复验；Agent 必须解释改动并保留错误前后对比。
...
- 浏览器 E2E：使用鼠标完成第 4 节全部步骤，包括自由布置场景、主动构建、模拟和错误修复。
...
- 家庭寻宝和博物馆导览共用架构工作台、通用场景图元、编译器和模拟器，不出现任务 ID 分支或专属道具组件。
...
- 无约束的开放式 Agent 对话，以及让大模型直接生成或执行 `RobotPlan`。
- 真实小车执行及完整家庭寻宝课程。
- 儿童语音输入的 STT/TTS 服务实现；P3 只保留 `ChildSpeechInputAdapter`。
- 灯光、音乐等尚未开放的 Tool 及其模拟能力。
- 云同步、账号、多人协作和家长端。
- iPad 原生识别桥。
- 完整课程系统、自由工坊和商业级视觉包装。
- 三维全息编辑器。
