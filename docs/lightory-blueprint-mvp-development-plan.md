...
-> 确定性编译 -> RobotPlan -> 双重安全校验 -> 通用二维试验场
```

实现：

- 将连线统一为儿童可理解的“交付连接”：交付成果或“已完成”信号。默认显示自然语言标签，技术端口和数据类型按需展开。
...
- 试验场通过“工程总图 / 二维试验场”切换进入，首批已实现 8×6 米场地、小车起点、目标/地标、障碍物、拖动、尺寸/朝向/语义编辑、撤销重做、边界约束和刷新恢复。场景修改独立于 AgentWorkflow，不触发 Agent 重建。
...
9. [完成] 增加可编辑的开始/结束模块、连接约束和默认安全停止；提高二维试验场右侧信息栏的平板可读字号。
