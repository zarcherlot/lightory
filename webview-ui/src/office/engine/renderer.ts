...
function renderRoleTaskIcons(
  ...
  for (const ch of characters) {
    if (ch.roleTaskState !== 'busy' && ch.roleTaskState !== 'weather') continue;
    const sittingOff =
      ch.state === CharacterState.TYPE || ch.state === CharacterState.BUSY
        ? BUBBLE_SITTING_OFFSET_PX
        : 0;
    const cx = Math.round(offsetX + ch.x * zoom);
    const cy = Math.round(offsetY + (ch.y + sittingOff - BUBBLE_VERTICAL_OFFSET_PX) * zoom);
    const size = Math.max(18, Math.round(14 * zoom));
    const x = cx - size / 2;
    const y = cy - size - 2 * zoom;

    ctx.save();
    ctx.fillStyle = ROLE_ICON_BG;
    ctx.strokeStyle = ROLE_ICON_BORDER;
    ctx.lineWidth = Math.max(2, Math.round(zoom));
    ctx.beginPath();
    ctx.roundRect(x, y, size, size, Math.max(3, Math.round(2 * zoom)));
    ctx.fill();
    ctx.stroke();
...
function drawWeatherIcon(
  ...
  if (icon === 'snow') {
    ...
    drawDrops(ctx, x, y, size, ROLE_ICON_SNOW, true);
    return;
  }
  if (icon === 'rain') {
    drawCloud(ctx, x, y, size);
    drawDrops(ctx, x, y, size, ROLE_ICON_RAIN, false);
    return;
  }
  if (icon === 'storm') {
    drawCloud(ctx, x, y, size);
    ctx.fillStyle = ROLE_ICON_STORM;
    ctx.beginPath();
    ctx.moveTo(x + size * 0.48, y + size * 0.5);
    ...
    ctx.lineTo(x + size * 0.52, y + size * 0.64);
    ctx.closePath();
    ctx.fill();
    return;
  }
...
function drawSun(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
  const cx = x + size / 2;
  const cy = y + size / 2;
  ctx.strokeStyle = ROLE_ICON_SUN_RAY;
  ctx.lineWidth = Math.max(1, size * 0.08);
  for (let i = 0; i < 8; i++) {
    const a = (Math.PI * 2 * i) / 8;
    ctx.beginPath();
    ctx.moveTo(cx + Math.cos(a) * size * 0.25, cy + Math.sin(a) * size * 0.25);
    ctx.lineTo(cx + Math.cos(a) * size * 0.38, cy + Math.sin(a) * size * 0.38);
    ctx.stroke();
  }
  ctx.fillStyle = ROLE_ICON_SUN;
  ctx.beginPath();
...
function drawCloud(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
  ctx.fillStyle = ROLE_ICON_CLOUD;
  ctx.beginPath();
  ctx.arc(x + size * 0.38, y + size * 0.52, size * 0.16, Math.PI, Math.PI * 2);
  ctx.arc(x + size * 0.52, y + size * 0.43, size * 0.2, Math.PI, Math.PI * 2);
  ctx.arc(x + size * 0.66, y + size * 0.54, size * 0.15, Math.PI, Math.PI * 2);
  ctx.lineTo(x + size * 0.78, y + size * 0.64);
  ctx.lineTo(x + size * 0.25, y + size * 0.64);
  ctx.closePath();
  ctx.fill();
}
...
function drawDrops(
  ...
  ctx.fillStyle = color;
  for (const dx of [0.38, 0.52, 0.66]) {
    if (snow) {
      ctx.fillRect(
        x + size * dx,
        y + size * 0.72,
      ...
      ctx.beginPath();
      ctx.ellipse(x + size * dx, y + size * 0.76, size * 0.035, size * 0.09, 0, 0, Math.PI * 2);
      ctx.fill();
    }
  }
}
...
export function renderFrame(
  ...
  renderRoleTaskIcons(ctx, characters, offsetX, offsetY, zoom);
