/**
 * AgentRuntime: agent lifecycle core for the browser server.
 *
 * Owns timer Maps, file watchers, HookEventHandler, DismissalTracker, session
 * scanning, and agent removal. The CLI creates an instance and registers
 * lifecycle callbacks.
 *
...
export class AgentRuntime {
  ...
  private shouldTrackExternalHookSession(projectDir: string, cwd: string): boolean {
   ...
   * Restored agents are hooks/external sessions; terminal-owned sessions are
   * detected live through the provider scanner when available.
   */
