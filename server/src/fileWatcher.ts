...
/** Register the terminal adapter when a provider can expose external terminals. */
export function setTerminalAdapter(adapter: ITerminalAdapter): void {
  terminalAdapter = adapter;
}

/** Agent removal callback injected by the runtime owner. */
let agentRemovalCallback: ((id: number) => void) | null = null;

/** Register the agent removal callback. */
...
function adoptExternalSession(
  ...
  // synthetic scenarios: tool_use at t=1s, scanner ticks at t=3s). If we
...
/**
 * Periodically scans for external sessions.
