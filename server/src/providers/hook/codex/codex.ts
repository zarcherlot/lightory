...
function getTool(raw: Record<string, unknown>): { id: string; name: string; input: unknown } {
  ...
  const input =
    raw.tool_input ?? raw.input ?? raw.arguments ?? tool.input ?? call.input ?? action.input ?? {};
  ...
  return {
    ...
    name:
      firstString(raw.tool_name, raw.toolName, raw.name, tool.name, call.name, action.name) ??
      'tool',
    input,
  };
}
